...
function Nav({ onCartOpen }) {
  // Destructure values from context (ensure correct names)
  const { input, setInput, cat, setCat, foodItems } = useContext(dataContext);
  ...
  useEffect(() => {
    if (foodItems && foodItems.length > 0) {
      const newList = foodItems.filter((food) => {
        // Ensure 'food' and 'item' are defined before calling toLowerCase()
        return (
          food?.item && // Optional chaining for safety
          food.item.toLowerCase().includes(input.toLowerCase())
        );
      });
      setCat(newList);
    }
  }, [input, foodItems, setCat]);
